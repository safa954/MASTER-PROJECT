...
if(isset($_GET['trans_truck_id'])){
    $trans_truck_id = $_GET['trans_truck_id'];
}
$username = $password = $confirm_password = $email = $phone_num =$address = $full_name= "";
$username_err  = $email_arr = $phone_num_err = $full_name_arr = "";

$sql = "select * from transport_truck where trans_truck_id = '".$trans_truck_id."'";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) &&empty($phone_number_err)  && empty($address_err)){
        ...
        $sql = "UPDATE transport_truck SET name=?, phone_number=?, address=? WHERE trans_truck_id=?";

//        $sql = "INSERT INTO users (username, password,email,phone_num) VALUES (?,?,?,?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssd", $param_name, $param_phone_number,$param_address,$param_trans_truck_id);

            // Set parameters
            $param_trans_truck_id = $trans_truck_id;
            $param_name = $name;
                        ...
                        <div class="card-block">
                           <form method="post">
                              <div class="form-group">
                                 ...
                                 <input type="text" class="form-control <?php echo (!empty($name_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $name?>" name="name" placeholder="Enter name">
                                 <span class="invalid-feedback"><?php echo $name_err; ?></span>

                              </div>
                              <div class="form-group">
                                 <label for="name" class="form-control-label">Phone Number</label>
                                 <input type="text" class="form-control <?php echo (!empty($phone_number_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $phone_number?>" name="phone_number" placeholder="Enter phone_number">
